  ...
  root.innerHTML = `
    <div class="topbar catalog-topbar">
      <div class="brand">
        <div class="logo"></div>
        <div class="title">
          <b id="title">Способы оплат</b>
          <div class="muted">нал/безнал/СБП/…</div>
        </div>
      </div>
    ...
    <div class="card catalog-hero">
      <div class="muted catalog-intro">Настрой способы оплат, которые будут в отчёте закрытия смены. По умолчанию создаются: Наличные, Безналичные, СБП, Прочее.</div>
    ...
    <div id="editModal" class="modal">
      ...
      <div class="modal__panel">
        <div class="modal__head">
          <div>
            <b class="modal__title" id="editTitle">Способ оплаты</b>
...
function openEditModal({ title, hint, bodyHtml }) {
  ...
  if (t) t.textContent = title || "Способ оплаты";
...
function computeCaps(perms) {
  ...
  return {
    view: owner || has(perms, "PAYMENT_METHODS_VIEW"),
    create: owner || has(perms, "PAYMENT_METHODS_CREATE"),
    edit: owner || has(perms, "PAYMENT_METHODS_EDIT"),
    archive: owner || has(perms, "PAYMENT_METHODS_ARCHIVE"),
  };
}
...
function renderList() {
  ...
  if (!state.can.view) {
    el.innerHTML = `<div class="catalog-state catalog-state--denied"><b>Нет доступа к способам оплаты</b><span>Обратитесь к владельцу заведения, чтобы получить право просмотра.</span></div>`;
    return;
  }

  if (state.loadError) {
    el.innerHTML = `<div class="catalog-state catalog-state--error"><b>Не удалось загрузить способы оплаты</b><span>${esc(state.loadError)}</span></div>`;
    return;
  }

  if (!state.items.length) {
    el.innerHTML = `<div class="catalog-state catalog-state--empty"><b>Способов оплаты пока нет</b><span>Добавьте первый способ оплаты для отчёта закрытия смены.</span></div>`;
    return;
  }
  ...
  for (const it of state.items) {
    ...
    left.innerHTML = `
      ...
      <div class="muted catalog-row__meta">${it.is_active ? "Доступен при выборе оплаты" : "Скрыт из списка"}</div>
    `;
    ...
    if (state.can.archive) {
      ...
      btn.onclick = async () => {
        const ok = await confirmModal({
          title: it.is_active ? "Архивировать способ оплаты?" : "Восстановить способ оплаты?",
        ...
        if (!ok) return;
        try {
          await updatePaymentMethod(state.venueId, it.id, { is_active: !it.is_active });
          toast("Готово", "ok");
          await load();
        } catch (e) {
  ...
  return `
    <div class="grid grid2 catalog-form">
      <div>
        ...
        <input id="f_title" placeholder="Наличные" value="${esc(it.title || "")}" />
      </div>
      <div>
...
function wireEditor({ mode, item }) {
  ...
  document.getElementById("btnSave")?.addEventListener("click", async () => {
    ...
    const code = ensureUniqueCode(slugifyCode(title, "payment_method"), state.items, item?.id ?? null);
    ...
    try {
      if (mode === "create") {
        await createPaymentMethod(state.venueId, payload);
      } else {
        await updatePaymentMethod(state.venueId, item.id, payload);
      }
      closeEditModal();
      toast("Сохранено", "ok");
      await load();
...
function openEditor({ mode, item }) {
  ...
  openEditModal({
    title: mode === "create" ? "Новый способ оплаты" : "Способ оплаты",
    hint: mode === "create" ? "Добавь способ оплаты для отчёта закрытия смены" : "Редактирование способа оплаты",
...
async function load() {
  ...
  try {
    const items = await getPaymentMethods(state.venueId, { includeArchived: state.includeArchived });
