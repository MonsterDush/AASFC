...
function renderShell() {
  root.innerHTML = `
    ...
    <div class="card catalog-hero">
      <div class="muted catalog-intro">Сохраните поставщиков и контакты, чтобы расходы одинаково оформлялись во всех сменах.</div>
    ...
    <div id="editModal" class="modal">
      ...
      <div class="modal__panel">
        <div class="modal__head">
          <div><b class="modal__title" id="editTitle">Поставщик</b><div class="muted small mt-4" id="editHint"></div></div>
...
function renderList() {
  ...
  if (!el) return;
  if (!state.canView) { el.innerHTML = `<div class="catalog-state catalog-state--denied"><b>Нет доступа к поставщикам</b><span>Обратитесь к владельцу заведения, чтобы получить право просмотра.</span></div>`; return; }
  if (state.loadError) { el.innerHTML = `<div class="catalog-state catalog-state--error"><b>Не удалось загрузить поставщиков</b><span>${esc(state.loadError)}</span></div>`; return; }
  if (!state.items.length) { el.innerHTML = `<div class="catalog-state catalog-state--empty"><b>Поставщиков пока нет</b><span>Добавьте первого поставщика, чтобы использовать его при учёте расходов.</span></div>`; return; }
  el.innerHTML = state.items.map((it) => `
    <div class="catalog-row">
      <div class="catalog-row__copy">
        ...
        <div class="muted catalog-row__meta">${esc(it.contact || "Контакты не указаны")}</div>
      </div>
...
function editorForm(item = null) {
  ...
  return `
    <div class="grid grid2 catalog-form">
      ...
      <div>
        <div class="muted mb-6">Контакт</div>
