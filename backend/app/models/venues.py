...
def _get_position(db: Session, *, venue_id: int, user: User) -> VenuePosition | None:
    return db.execute(
        select(VenuePosition).where(
            VenuePosition.venue_id == venue_id,
            VenuePosition.member_user_id == user.id,
...
def _is_reports_editor(db: Session, *, venue_id: int, user: User) -> bool:
    if _is_owner_or_super_admin(db, venue_id=venue_id, user=user):
        return True
    pos = _get_position(db, venue_id=venue_id, user=user)
    return bool(pos and pos.can_make_reports)


def _require_reports_editor(db: Session, *, venue_id: int, user: User) -> None:
    if not _is_reports_editor(db, venue_id=venue_id, user=user):
        raise HTTPException(status_code=403, detail="Forbidden")


def _is_reports_viewer(db: Session, *, venue_id: int, user: User) -> bool:
    # Owners/admins can always view
    if _is_active_member_or_admin(db, venue_id=venue_id, user=user) and user.system_role in ("SUPER_ADMIN", "MODERATOR"):
        return True
    if _is_owner_or_super_admin(db, venue_id=venue_id, user=user):
        return True
    pos = _get_position(db, venue_id=venue_id, user=user)
...
def _require_reports_viewer(db: Session, *, venue_id: int, user: User) -> None:
    if not _is_reports_viewer(db, venue_id=venue_id, user=user):
        raise HTTPException(status_code=403, detail="Forbidden")
...
def delete_venue(
    ...
    # Clean up dependent rows first to avoid FK issues (and SQLAlchemy trying to nullify FK columns).
    # NOTE: We use bulk deletes for MVP simplicity.
    # Order matters: assignments -> shifts -> intervals -> positions -> members -> invites -> venue.
    shift_ids = [r[0] for r in db.query(Shift.id).filter(Shift.venue_id == venue_id).all()]
    if shift_ids:
        db.query(ShiftAssignment).filter(ShiftAssignment.shift_id.in_(shift_ids)).delete(synchronize_session=False)
        db.query(Shift).filter(Shift.id.in_(shift_ids)).delete(synchronize_session=False)

    db.query(ShiftInterval).filter(ShiftInterval.venue_id == venue_id).delete(synchronize_session=False)
    db.query(VenuePosition).filter(VenuePosition.venue_id == venue_id).delete(synchronize_session=False)
    db.query(VenueInvite).filter(VenueInvite.venue_id == venue_id).delete(synchronize_session=False)
    db.query(VenueMember).filter(VenueMember.venue_id == venue_id).delete(synchronize_session=False)
    db.query(DailyReport).filter(DailyReport.venue_id == venue_id).delete(synchronize_session=False)
...
def upsert_daily_report(
    ...
    _require_reports_editor(db, venue_id=venue_id, user=user)
    ...
    if existing is None:
        ...
        db.commit()
        db.refresh(rep)
        return {"id": rep.id, "mode": "created"}
    else:
        existing.cash = payload.cash
        existing.cashless = payload.cashless
        existing.revenue_total = payload.revenue_total
...
def list_daily_reports(
    ...
    _require_reports_viewer(db, venue_id=venue_id, user=user)
    ...
    can_rev = _can_view_revenue(db, venue_id=venue_id, user=user)

    rows = db.execute(
        select(DailyReport).where(DailyReport.venue_id == venue_id, DailyReport.date >= start, DailyReport.date < end)
    ...
    return [
        {
            ...
            "report_exists": True,
            "cash": int(r.cash) if can_rev else None,
            "cashless": int(r.cashless) if can_rev else None,
            "revenue_total": int(r.revenue_total) if can_rev else None,
...
def get_daily_report(
    ...
    return {
        ...
        "report_exists": True,
        "cash": int(rep.cash) if can_rev else None,
        "cashless": int(rep.cashless) if can_rev else None,
        "revenue_total": int(rep.revenue_total) if can_rev else None,
        "created_by": rep.created_by_user_id,
        "created_at": rep.created_at.isoformat() if rep.created_at else None,
        "updated_by": rep.updated_by_user_id,
