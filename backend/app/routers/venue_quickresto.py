...
from app.services.integrations.quickresto_import_batches import (
    active_quickresto_import_batch,
    create_quickresto_import_batch,
    latest_quickresto_import_batch,
    retry_quickresto_import_batch,
    run_quickresto_import_batch_worker,
    serialize_quickresto_import_batch,
)
...
def _require_quickresto_idle(db: Session, connection: QuickRestoConnection) -> None:
    if active_quickresto_import_batch(db, connection_id=int(connection.id)) is not None:
        raise HTTPException(
            status_code=409,
            detail="Помесячный импорт QuickResto выполняется. Дождитесь его завершения перед изменением интеграции.",
        )
...
def _serialize_import_batch(db: Session, batch: QuickRestoImportBatch) -> dict:
    result = serialize_quickresto_import_batch(batch)
    ...
    for value in summary.get("issue_ids") or []:
        try:
            ...
            continue
    if batch.last_sync_run_id is not None:
        issue_ids.update(
            int(value)
            for value in db.execute(
                select(QuickRestoImportIssue.id).where(
                    QuickRestoImportIssue.connection_id == int(batch.connection_id),
                    QuickRestoImportIssue.last_sync_run_id == int(batch.last_sync_run_id),
                )
            ).scalars()
        )
    result["issue_ids"] = sorted(issue_id for issue_id in issue_ids if issue_id > 0)
    return result
...
def get_quickresto_connection(
    ...
    return {
        ...
        "import_batch": _serialize_import_batch(db, latest_batch) if latest_batch is not None else None,
    }
...
def post_quickresto_sync(
    ...
    _require_quickresto_manage(db, venue_id=venue_id, user=user)
    connection = _connection_or_404(db, venue_id)
    if connection.sync_from_date is not None and pending_quickresto_scope(connection) is None:
        try:
            ...
            raise HTTPException(status_code=409, detail=str(exc)) from exc
        background_tasks.add_task(run_quickresto_import_batch_worker, int(batch.id))
        return {
            "ok": True,
            "queued": True,
            "import_batch": _serialize_import_batch(db, batch),
        }
    try:
...
@router.get("/{venue_id}/integrations/quickresto/import-batches")
def list_quickresto_import_batches(
    venue_id: int,
    ...
    rows = list(
        db.execute(
            statement.order_by(
                QuickRestoImportBatch.created_at.desc(),
                QuickRestoImportBatch.id.desc(),
            ).limit(limit)
        ).scalars()
    )
    return {"batches": [_serialize_import_batch(db, item) for item in rows]}


@router.post("/{venue_id}/integrations/quickresto/import-batches/{batch_id}/retry")
def post_quickresto_import_batch_retry(
    venue_id: int,
    batch_id: int,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
    ...
    batch = db.execute(
        select(QuickRestoImportBatch).where(
            QuickRestoImportBatch.id == int(batch_id),
            QuickRestoImportBatch.connection_id == int(connection.id),
        )
    ).scalar_one_or_none()
    if batch is None:
        raise HTTPException(status_code=404, detail="QuickResto import batch not found")
    try:
        batch = retry_quickresto_import_batch(db, batch=batch)
    except QuickRestoSyncError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    background_tasks.add_task(run_quickresto_import_batch_worker, int(batch.id))
    return {"ok": True, "import_batch": _serialize_import_batch(db, batch)}
