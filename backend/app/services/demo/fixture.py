    ...
    for column in plan.model.__table__.columns:
        ...
        if column.name in USER_REFERENCE_COLUMNS and value is not None and int(value) not in allowed_user_ids:
            if demo_owner_user_id is not None:
                warnings.append(
                    f"{plan.name}.{column.name}: заменён внешний user_id={value} на demo owner {demo_owner_user_id}"
                )
...
def _collect_live_context(db: Session, venue_id: int) -> dict[str, Any]:
    ...
    ctx["shift_comment_ids"] = load_ids(
        ShiftComment.__table__, _in_ids(ShiftComment.__table__.c.shift_id, ctx["shift_ids"])
    )
    ...
    ctx["adjustment_dispute_ids"] = load_ids(
        AdjustmentDispute.__table__, AdjustmentDispute.__table__.c.venue_id == int(venue_id)
    )
    return ctx
...
def _collect_reset_context(db: Session, fixture: dict[str, Any], venue_id: int) -> dict[str, Any]:
    ...
    fixture_user_ids = {
        int(row["id"]) for row in fixture.get("tables", {}).get("users", []) if row.get("id") is not None
    }
    ...
    if ctx.get("demo_owner_user_id") is None:
        owner_rows = [
            row
            for row in fixture.get("tables", {}).get("users", [])
            if str(row.get("demo_persona") or "").upper() == DEMO_PERSONA_OWNER
        ]
        if owner_rows:
...
def _fixture_table_plans() -> list[FixtureTablePlan]:
    return [
        ...
        FixtureTablePlan(
            "users",
            User,
            lambda c, t: _in_ids(t.c.id, c["fixture_user_ids"]),
            lambda c, t: t.c.is_demo_user.is_(True) & _in_ids(t.c.id, c["all_demo_user_ids"]),
        ),
        FixtureTablePlan(
            "venue_members",
            VenueMember,
            lambda c, t: (t.c.venue_id == c["venue_id"]) & _in_ids(t.c.user_id, c["fixture_user_ids"]),
            lambda c, t: (t.c.venue_id == c["venue_id"]) & _in_ids(t.c.user_id, c["all_demo_user_ids"]),
        ),
        FixtureTablePlan(
            "venue_positions",
            VenuePosition,
        ...
        FixtureTablePlan(
            ...
            ShiftCommentMention,
            lambda c, t: _in_ids(t.c.comment_id, c["shift_comment_ids"]),
            lambda c, t: _in_ids(t.c.comment_id, c["shift_comment_ids"]),
        ),
        FixtureTablePlan(
            "daily_reports",
        ...
        FixtureTablePlan(
            "daily_report_values",
            DailyReportValue,
            lambda c, t: _in_ids(t.c.report_id, c["daily_report_ids"]),
            lambda c, t: _in_ids(t.c.report_id, c["daily_report_ids"]),
        ),
        FixtureTablePlan(
            "daily_report_audits",
        ...
        FixtureTablePlan(
            ...
            RecurringExpenseRulePaymentMethod,
            lambda c, t: _in_ids(t.c.rule_id, c["recurring_rule_ids"]),
            lambda c, t: _in_ids(t.c.rule_id, c["recurring_rule_ids"]),
        ),
        FixtureTablePlan(
        ...
        FixtureTablePlan(
            "expenses", Expense, lambda c, t: t.c.venue_id == c["venue_id"], lambda c, t: t.c.venue_id == c["venue_id"]
        ),
        FixtureTablePlan(
            "expense_allocations",
            ExpenseAllocation,
        ...
        FixtureTablePlan(
            ...
            AdjustmentDisputeComment,
            lambda c, t: _in_ids(t.c.dispute_id, c["adjustment_dispute_ids"]),
            lambda c, t: _in_ids(t.c.dispute_id, c["adjustment_dispute_ids"]),
        ),
        FixtureTablePlan(
            "venue_billing_state",
...
def export_demo_fixture(db: Session, *, venue_id: int, fixture_path: str | None = None) -> DemoFixtureExportResult:
    ...
    for plan in plans:
        ...
        rows = (
            db.execute(select(table).where(plan.export_where(ctx, table)).order_by(table.c.id.asc())).mappings().all()
        )
        serialized = [
            _sanitize_export_row(
                plan,
                dict(row),
                allowed_user_ids=set(ctx["fixture_user_ids"]),
                demo_owner_user_id=ctx.get("demo_owner_user_id"),
                warnings=warnings,
            )
            for row in rows
        ]
...
def reset_demo_fixture(
    db: Session, *, fixture_path: str | None = None, venue_id: int | None = None
...
def get_demo_fixture_status(db: Session, *, fixture_path: str | None = None) -> dict[str, Any]:
    ...
    venue = (
        db.execute(
            select(Venue.__table__)
            .where(Venue.__table__.c.is_demo.is_(True))
            .order_by(Venue.__table__.c.id.asc())
            .limit(1)
        )
        .mappings()
        .first()
    ...
    if venue_id is not None:
        ...
        owner_exists = (
            db.execute(
                ...
                .select_from(member_tbl.join(user_tbl, user_tbl.c.id == member_tbl.c.user_id))
                .where(
                    member_tbl.c.venue_id == venue_id,
                    user_tbl.c.is_demo_user.is_(True),
                    user_tbl.c.demo_persona == "OWNER",
                )
                .limit(1)
            ).first()
            is not None
