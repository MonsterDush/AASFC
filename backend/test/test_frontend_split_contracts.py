...
class QuickRestoIntegrationContractTests(TestCase):
    def test_owner_only_integration_hub_and_quickresto_page_use_venue_api(self):
        ...
        self.assertIn("20260909-pos3", hub_html)
        ...
        self.assertIn('id="iikoApiLogin"', hub_html)
        self.assertIn('id="iikoOrganizationId"', hub_html)
        self.assertIn('id="iikoCapabilityHint"', hub_html)
        self.assertIn('id="historicalIiko"', hub_html)
        self.assertNotIn("Интеграция с iiko в разработке", hub_html)
        self.assertIn("grid-template-columns:repeat(2,minmax(0,1fr))", hub_styles)
        self.assertIn("/integrations/quickresto`)", hub_script)
        self.assertIn("/integrations/iiko`)", hub_script)
        self.assertIn("/integrations/iiko/probe`", hub_script)
        self.assertIn("/integrations/iiko/sync/historical/enqueue`", hub_script)
        self.assertIn("/integrations/iiko/sync/incremental`", hub_script)
        self.assertIn("P0_CAPABILITIES", hub_script)
        self.assertIn("Импорт не будет запущен частично", hub_script)
        self.assertIn(".integrations-iiko-state", hub_styles)
...
class OwnerPayProfileSplitContractTests(TestCase):
    def test_profile_page_keeps_component_and_assignment_controllers(self):
        ...
        for filename, (factory, line_limit) in modules.items():
            ...
            cache_key = "20260723-functional1" if filename == "assignment-controller.js" else "20260906-tiers1"
            self.assertIn(f"/owner-pay-profile/{filename}?v={cache_key}", main)
            self.assertIn(f"export function {factory}", source)

        tiers = (FRONTEND / "owner-pay-profile" / "percent-tiers.js").read_text(encoding="utf-8")
        controller = (FRONTEND / "owner-pay-profile" / "component-controller.js").read_text(encoding="utf-8")
        self.assertLess(len(tiers.splitlines()), 260)
        self.assertIn("./percent-tiers.js?v=20260906-tiers1", controller)
