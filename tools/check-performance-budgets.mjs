...
for (const filePath of walk(frontendRoot)) {
  ...
  const effectiveMaximum = Number(
    budgets.assets.maxBytesByPath?.[relative] || maximum,
  );
  assert.ok(
    bytes <= effectiveMaximum,
    `${relative}: ${bytes} bytes exceeds the ${effectiveMaximum}-byte source budget`,
  );
  measured.push({ file: relative, bytes, maximum: effectiveMaximum });
}
...
const largest = measured
  ...
  .slice(0, 8);
console.log(
  JSON.stringify({ ok: true, checked: measured.length, largest }, null, 2),
);
