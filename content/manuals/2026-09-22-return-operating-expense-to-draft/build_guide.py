...
def confirmed_row(draw, top=584, highlight=None):
    ...
    BASE.normal_button(draw, (650, top + 165, 792, top + 213), "Изменить", size=10)
    button(draw, (808, top + 165, 960, top + 213), "Удалить", kind="danger", size=10)
    targets = {"details": (110, top + 105, 630, top + 273), "draft": (616, top + 97, 786, top + 161)}
    if highlight:
        draw.rounded_rectangle(targets[highlight], radius=17, outline=PURPLE, width=4)
...
def build_cover():
    ...
    draw_brand(img)
    draw.multiline_text((62, 175), "Как вернуть\nоперационный расход\nв черновик?", font=font(45, True), fill=NAVY, spacing=3)
    fit_text(draw, "Уберите подтверждённый документ из признания за месяц.", (66, 423), 480, 27, TEXT, spacing=7)
    fit_text(draw, "Полная сумма останется в строке расхода.", (66, 640), 470, 26, TEXT, spacing=7)
...
def build_step_2():
    ...
    panel_text(draw, (806, 568), "Записей", 12, False, PANEL_MUTED); panel_text(draw, (806, 599), "8", 29, True)
...
def build_step_3():
    ...
    draw = ImageDraw.Draw(img); list_header(draw, "8", "235 326,35 ₽", "только подтверждённые", "операционные расходы"); confirmed_row(draw, highlight="details")
    draw_bottom_note(img, "Подтверждённый расход уже участвует в признании за месяц."); save(img, "04-step.png")


def build_step_4():
    img = base_canvas(); draw_step_header(img, "4", "ШАГ 4", "Верните в черновик", "Нажмите ghost-кнопку в строке расхода."); draw_ui_frame(img)
    draw = ImageDraw.Draw(img); list_header(draw, "8", "235 326,35 ₽", "только подтверждённые", "операционные расходы"); confirmed_row(draw, highlight="draft")
    draw_bottom_note(img, "«В черновик» — тёмная контурная ghost-кнопка."); save(img, "05-step.png")
...
def build_step_5():
    ...
    draw = ImageDraw.Draw(img); list_header(draw, "7", "228 478,75 ₽", "только подтверждённые", "операционные расходы")
...
def build_step_6():
    img = base_canvas(); draw_step_header(img, "6", "ШАГ 6", "Покажите черновики", "Смените фильтр статуса, чтобы увидеть результат."); draw_ui_frame(img)
    draw = ImageDraw.Draw(img); expenses_header(draw)
    ...
    draw_bottom_note(img, "Теперь расход отображается среди черновиков."); save(img, "07-step.png")


def build_result():
    img = base_canvas(); draw_step_header(img, "✓", "ГОТОВО", "Расход в черновике", "Сумма больше не признана в выбранном месяце."); draw_ui_frame(img)
    draw = ImageDraw.Draw(img); list_header(draw, "1", "228 478,75 ₽", "только черновики", "операционные расходы"); draft_row(draw, highlight="details")
    draw_bottom_note(img, "Доступны «Подтвердить», «Отменить», «Изменить» и «Удалить»."); save(img, "08-result.png")
